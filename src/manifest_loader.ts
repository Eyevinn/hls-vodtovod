...
export class FileManifestLoader extends AbstractManifestParser implements IManifestLoader {
  load(uri: string): Promise<any> {
    return new Promise((resolve, reject) => {
      try {
        const stream = createReadStream(uri);
        this.parse(stream).then(resolve);
      } catch (err) {
        reject(`Failed to read manifest: ` + err);
      }
    });
  }
}
